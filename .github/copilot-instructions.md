# GitHub Copilot Instructions for Swipeclock Scripting Language

## Language Overview
TimeWorksPlus scripting is a custom scripting language used in Swipeclock's TimeWorksPlus and WorkforceHub software products. This language is case-insensitive and has unique syntax rules that differ significantly from standard programming languages.
...
// CORRECT - string operators (not functions)
if (employee.department contains "OPS") {
  addalert("Contains OPS");
}
```
...
// WRONG - using function syntax for operators
contains(employee.department, "OPS")  // Should be: employee.department contains "OPS"
```
...
These file extensions are all recognized as Swipeclock scripting language. Apply the same language rules to any of them:

- `.twp` - TimeWorksPlus script files
- `.twpscript` - Alternative extension for TimeWorksPlus scripts
- `.swipeclock` - Swipeclock script files
- `.scscript` - Swipeclock script files (alternative)
- `.wfhscript` - WorkforceHub script files
- `.wfhub` - WorkforceHub script files
- `.workforcehub` - WorkforceHub script files
